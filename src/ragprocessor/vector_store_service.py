...
if __name__ == "__main__":
    try:
        ...
        # Example query (always try this)
        print(f"\nTesting query functionality...")
        results = vector_store_service.query("Z którego roku pochodził numer 5?")
        
        print(f"\nQuery Results:")
        print(f"Found {len(results)} results")
        if results:
            for i, result in enumerate(results, 1):  # Show top 3
        ...
        if collection_info["exists"]:
            ...
            print(f"  Collection status: {collection_info.get('message', 'Does not exist')}")
